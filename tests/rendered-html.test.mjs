...
test("server-renders the MangaMori discovery experience", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);

  const html = await response.text();
  assert.match(html, /<title>MangaMori — Anime &amp; Manhwa Empfehlungen<\/title>/i);
  assert.match(html, /Dein nächstes/);
